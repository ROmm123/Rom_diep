...
class enemy_player():
    def __int__(self,settings,player):
        self.set = settings
        self.Playerrr = player

    def Enemy_player(self , x , y , radius , color):
    ...
    def calculate(self,data):
        ...
        b1 = k1 - int(player.center_x)
        b2 = k2 - int(player.center_y)
        a1 = abs(k1 - int(player.center_x))
        a2 = abs(k2 - int(player.center_y))
    ...
    def check(self,a1,a2,properties):
        if a2 < self.settings.screen_height and a1 < self.settings.screen_width:
            ...
            self.color=color_from_packet
            self.center_x=properties[5]
            self.center_x=properties[6]
            self.draw()
        else:
            print('0')
