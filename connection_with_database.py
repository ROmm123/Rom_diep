...
class CdataB():
    ...
    def handle_data_for_signin(self, username, password):
        try:
    ...
    def handle_data_forLogin(self):
        flag = False
        while True:
            if flag and self.Queue.empty():
                break
    ...
    def handle_data_for_leavegame(self, username, password, x, y, speedCounter, sizeCounter, shieldCounter, HPCounter):
        try:
            conn = mysql.connector.connect(
                host='127.0.0.1',  # Host address
                port='3306',  # Port number
